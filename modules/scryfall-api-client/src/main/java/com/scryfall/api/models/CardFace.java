/**
 * Code generated by Microsoft (R) AutoRest Code Generator.
 * Changes may cause incorrect behavior and will be lost if the code is
 * regenerated.
 */

package com.scryfall.api.models;

import com.fasterxml.jackson.annotation.JsonProperty;
import com.scryfall.api.PersistedObject;

import javax.persistence.*;
import java.util.List;
import java.util.UUID;

/**
 * The CardFace model.
 */
@Entity
public class CardFace extends PersistedObject {

    /**
     * The name property.
     */
    @JsonProperty(value = "name")
    private String name;

    /**
     * The typeLine property.
     */
    @JsonProperty(value = "type_line")
    private String typeLine;

    /**
     * The oracleText property.
     */
    @JsonProperty(value = "oracle_text")
    @Column(name="oracle_text", length = 1000)
    private String oracleText;

    /**
     * The manaCost property.
     */
    @JsonProperty(value = "mana_cost")
    private String manaCost;

    /**
     * The colors property.
     */
    @JsonProperty(value = "colors")
    @ElementCollection
    @Enumerated(EnumType.STRING)
    private List<Colors> colors;

    /**
     * The colorIndicator property.
     */
    @JsonProperty(value = "color_indicator")
    @ElementCollection
    @Enumerated(EnumType.STRING)
    private List<Colors> colorIndicator;

    /**
     * The power property.
     */
    @JsonProperty(value = "power")
    private String power;

    /**
     * The toughness property.
     */
    @JsonProperty(value = "toughness")
    private String toughness;

    /**
     * The loyalty property.
     */
    @JsonProperty(value = "loyalty")
    private String loyalty;

    /**
     * The flavorText property.
     */
    @JsonProperty(value = "flavor_text")
    private String flavorText;

    /**
     * The illustrationId property.
     */
    @JsonProperty(value = "illustration_id")
    private UUID illustrationId;

    /**
     * The imageUris property.
     */
    @JsonProperty(value = "image_uris")
    @OneToOne(cascade = CascadeType.ALL)
    private ImageUri imageUris;

    /**
     * Get the name value.
     *
     * @return the name value
     */
    public String name() {
        return this.name;
    }

    /**
     * Set the name value.
     *
     * @param name the name value to set
     * @return the CardFace object itself.
     */
    public CardFace withName(String name) {
        this.name = name;
        return this;
    }

    /**
     * Get the typeLine value.
     *
     * @return the typeLine value
     */
    public String typeLine() {
        return this.typeLine;
    }

    /**
     * Set the typeLine value.
     *
     * @param typeLine the typeLine value to set
     * @return the CardFace object itself.
     */
    public CardFace withTypeLine(String typeLine) {
        this.typeLine = typeLine;
        return this;
    }

    /**
     * Get the oracleText value.
     *
     * @return the oracleText value
     */
    public String oracleText() {
        return this.oracleText;
    }

    /**
     * Set the oracleText value.
     *
     * @param oracleText the oracleText value to set
     * @return the CardFace object itself.
     */
    public CardFace withOracleText(String oracleText) {
        this.oracleText = oracleText;
        return this;
    }

    /**
     * Get the manaCost value.
     *
     * @return the manaCost value
     */
    public String manaCost() {
        return this.manaCost;
    }

    /**
     * Set the manaCost value.
     *
     * @param manaCost the manaCost value to set
     * @return the CardFace object itself.
     */
    public CardFace withManaCost(String manaCost) {
        this.manaCost = manaCost;
        return this;
    }

    /**
     * Get the colors value.
     *
     * @return the colors value
     */
    public List<Colors> colors() {
        return this.colors;
    }

    /**
     * Set the colors value.
     *
     * @param colors the colors value to set
     * @return the CardFace object itself.
     */
    public CardFace withColors(List<Colors> colors) {
        this.colors = colors;
        return this;
    }

    /**
     * Get the colorIndicator value.
     *
     * @return the colorIndicator value
     */
    public List<Colors> colorIndicator() {
        return this.colorIndicator;
    }

    /**
     * Set the colorIndicator value.
     *
     * @param colorIndicator the colorIndicator value to set
     * @return the CardFace object itself.
     */
    public CardFace withColorIndicator(List<Colors> colorIndicator) {
        this.colorIndicator = colorIndicator;
        return this;
    }

    /**
     * Get the power value.
     *
     * @return the power value
     */
    public String power() {
        return this.power;
    }

    /**
     * Set the power value.
     *
     * @param power the power value to set
     * @return the CardFace object itself.
     */
    public CardFace withPower(String power) {
        this.power = power;
        return this;
    }

    /**
     * Get the toughness value.
     *
     * @return the toughness value
     */
    public String toughness() {
        return this.toughness;
    }

    /**
     * Set the toughness value.
     *
     * @param toughness the toughness value to set
     * @return the CardFace object itself.
     */
    public CardFace withToughness(String toughness) {
        this.toughness = toughness;
        return this;
    }

    /**
     * Get the loyalty value.
     *
     * @return the loyalty value
     */
    public String loyalty() {
        return this.loyalty;
    }

    /**
     * Set the loyalty value.
     *
     * @param loyalty the loyalty value to set
     * @return the CardFace object itself.
     */
    public CardFace withLoyalty(String loyalty) {
        this.loyalty = loyalty;
        return this;
    }

    /**
     * Get the flavorText value.
     *
     * @return the flavorText value
     */
    public String flavorText() {
        return this.flavorText;
    }

    /**
     * Set the flavorText value.
     *
     * @param flavorText the flavorText value to set
     * @return the CardFace object itself.
     */
    public CardFace withFlavorText(String flavorText) {
        this.flavorText = flavorText;
        return this;
    }

    /**
     * Get the illustrationId value.
     *
     * @return the illustrationId value
     */
    public UUID illustrationId() {
        return this.illustrationId;
    }

    /**
     * Set the illustrationId value.
     *
     * @param illustrationId the illustrationId value to set
     * @return the CardFace object itself.
     */
    public CardFace withIllustrationId(UUID illustrationId) {
        this.illustrationId = illustrationId;
        return this;
    }

    /**
     * Get the imageUris value.
     *
     * @return the imageUris value
     */
    public ImageUri imageUris() {
        return this.imageUris;
    }

    /**
     * Set the imageUris value.
     *
     * @param imageUris the imageUris value to set
     * @return the CardFace object itself.
     */
    public CardFace withImageUris(ImageUri imageUris) {
        this.imageUris = imageUris;
        return this;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;
        if (!super.equals(o)) return false;

        CardFace cardFace = (CardFace) o;

        if (name != null ? !name.equals(cardFace.name) : cardFace.name != null) return false;
        if (typeLine != null ? !typeLine.equals(cardFace.typeLine) : cardFace.typeLine != null) return false;
        if (oracleText != null ? !oracleText.equals(cardFace.oracleText) : cardFace.oracleText != null) return false;
        if (manaCost != null ? !manaCost.equals(cardFace.manaCost) : cardFace.manaCost != null) return false;
        if (colors != null ? !colors.equals(cardFace.colors) : cardFace.colors != null) return false;
        if (colorIndicator != null ? !colorIndicator.equals(cardFace.colorIndicator) : cardFace.colorIndicator != null)
            return false;
        if (power != null ? !power.equals(cardFace.power) : cardFace.power != null) return false;
        if (toughness != null ? !toughness.equals(cardFace.toughness) : cardFace.toughness != null) return false;
        if (loyalty != null ? !loyalty.equals(cardFace.loyalty) : cardFace.loyalty != null) return false;
        if (flavorText != null ? !flavorText.equals(cardFace.flavorText) : cardFace.flavorText != null) return false;
        if (illustrationId != null ? !illustrationId.equals(cardFace.illustrationId) : cardFace.illustrationId != null)
            return false;
        return imageUris != null ? imageUris.equals(cardFace.imageUris) : cardFace.imageUris == null;
    }

    @Override
    public int hashCode() {
        int result = super.hashCode();
        result = 31 * result + (name != null ? name.hashCode() : 0);
        result = 31 * result + (typeLine != null ? typeLine.hashCode() : 0);
        result = 31 * result + (oracleText != null ? oracleText.hashCode() : 0);
        result = 31 * result + (manaCost != null ? manaCost.hashCode() : 0);
        result = 31 * result + (colors != null ? colors.hashCode() : 0);
        result = 31 * result + (colorIndicator != null ? colorIndicator.hashCode() : 0);
        result = 31 * result + (power != null ? power.hashCode() : 0);
        result = 31 * result + (toughness != null ? toughness.hashCode() : 0);
        result = 31 * result + (loyalty != null ? loyalty.hashCode() : 0);
        result = 31 * result + (flavorText != null ? flavorText.hashCode() : 0);
        result = 31 * result + (illustrationId != null ? illustrationId.hashCode() : 0);
        result = 31 * result + (imageUris != null ? imageUris.hashCode() : 0);
        return result;
    }
}
