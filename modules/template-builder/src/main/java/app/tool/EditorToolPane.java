...
public class EditorToolPane extends VBox {
    ...
    public void rebuild() {
        getChildren().clear();
        //Component Layout
        if (getTarget() instanceof Component component) {
            TitledPane layoutSection = new TitledPane("Component Layout", new HBox(
                    createTranslateControls(component),
                    createAnchorControls(component),
                    createButtonGrid(component)));
            layoutSection.setExpanded(true);
            getChildren().add(layoutSection);
        }

        //Properties
        if (!(getTarget() instanceof ImageComponent)) {
            return;
        }
        List<String> componentProps = List.of("image", "firstImage", "secondImage");
        ...
        ObservableList<PropertySheet.Item> properties = BeanPropertyUtils.getProperties(getTarget())
                .filtered(p -> componentProps.contains(p.getName()));

        for (PropertySheet.Item item : properties){
            if (!componentProps.contains(item.getName())){
                return;
            }
...
//        propertySheet.getItems().addAll(BeanPropertyUtils.getProperties(getTarget()));
    ...
    private PropertySheet createTranslateControls(Component component) {
        PropertySheet propertySheet = new PropertySheet();
        propertySheet.setPadding(Insets.EMPTY);
        propertySheet.setModeSwitcherVisible(false);
        propertySheet.setSearchBoxVisible(false);
        ObservableList<PropertySheet.Item> translateProps = BeanPropertyUtils.getProperties(component, p -> p.getName().startsWith("translate"));
        propertySheet.getItems().addAll(translateProps);

        return propertySheet;
    }

    private GridPane createAnchorControls(Component component) {
    ...
    private GridPane createButtonGrid(Component component) {
        GridPane gridPane = new GridPane();
